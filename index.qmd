...
ggplot(patients_df, aes(x = patient, y = post_surgical_score, 
  ...
  # Mean lines
  geom_hline(yintercept = mean_dreamy, color = "#4E79A7", linetype = "dashed", linewidth = 0.8) +
  geom_hline(yintercept = mean_duck, color = "#E15759", linetype = "dashed", linewidth = 0.8) +
  # Mean annotations
  annotate("text", x = max(patients_df$patient) * 0.95, y = mean_dreamy, 
           label = paste("○", round(mean_dreamy, 2)), 
           color = "#4E79A7", hjust = 1, vjust = -0.5, size = 3.5, fontface = "bold") +
  annotate("text", x = max(patients_df$patient) * 0.95, y = mean_duck, 
           label = paste("△", round(mean_duck, 2)), 
           color = "#E15759", hjust = 1, vjust = -0.5, size = 3.5, fontface = "bold") +
...
### Recover Figure 2: Randomized Assignment Outcomes
...
ggplot(patients_randomized_df, aes(x = patient, y = post_surgical_score, 
  ...
  geom_point(size = 2.5, alpha = 0.7) +
  # Mean lines
  geom_hline(yintercept = mean_dreamy_rand, color = "#4E79A7", linetype = "dashed", linewidth = 0.8) +
  geom_hline(yintercept = mean_duck_rand, color = "#E15759", linetype = "dashed", linewidth = 0.8) +
  # Mean annotations
...
### Recover Figure 3: Outcomes by Patient Severity (Stratification)
...
ggplot(patients_df, aes(x = severity, y = post_surgical_score, 
  ...
  geom_point(size = 2.5, alpha = 0.7) +
  # Mean lines
  geom_hline(yintercept = mean_dreamy_sev, color = "#4E79A7", linetype = "dashed", linewidth = 0.8) +
  geom_hline(yintercept = mean_duck_sev, color = "#E15759", linetype = "dashed", linewidth = 0.8) +
  # Mean annotations
